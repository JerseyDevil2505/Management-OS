...
const getPricePerUnit = useCallback((price, size) => {
  ...
  const saveAnalysis = useCallback(async (options = {}) => {
    ...
    try {
      ...
      if (!jobData?.id) {
        throw new Error('No job ID available for saving');
      }

      // Prepare sales data for saving - include manually added flag
      const salesWithMetadata = vacantSales.map(sale => ({
        ...sale,
        included: includedSales.has(sale.id),
        excluded: method1ExcludedSales.has(sale.id),
        category: saleCategories[sale.id] || null,
        special_region: specialRegions[sale.id] || 'Normal',
        notes: landNotes[sale.id] || null,
        manually_added: sale.manuallyAdded || false
      }));
      ...
      const analysisData = {
        ...
        vacant_sales_analysis: {
          ...
          analysis_summary: calculateRates()
        },
      ...
      // Use onAnalysisUpdate if available, otherwise direct supabase save
      if (typeof onAnalysisUpdate === 'function') {
        debug('💾 Saving via onAnalysisUpdate callback');
        await onAnalysisUpdate(analysisData, { source: options.source });
      } else {
        debug('💾 Saving directly to supabase');
      ...
      // Clear session state unsaved changes flag
      if (updateSessionState) {
        updateSessionState(prev => ({
          ...prev,
      ...
      if (options.source !== 'autosave') {
        ...
        setSaveRatesMessage('Analysis saved successfully');
        setTimeout(() => setShowSaveRatesNotification(false), 3000);
      }
    ...
    isSaving, jobData?.id, vacantSales, includedSales, method1ExcludedSales,
    ...
    computedAdjustments, onAnalysisUpdate, updateSessionState, calculateRates
  ]);

  // Expose saveAnalysis to window for auto-save
  useEffect(() => {
