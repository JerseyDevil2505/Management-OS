...
const DetailedAppraisalGrid = ({ result, jobData, codeDefinitions, vendorType, adjustmentGrid = [], compFilters = null, cmeBrackets = [], isJobContainerLoading = false, allProperties = [], marketLandData = {} }) => {
  ...
  const generatePDF = useCallback(async () => {
    ...
    // Render this page only if an appeal_log row exists for this subject. If the
    // appeal exists but no appellant_comps are supplied, we still render the page
    // with a "No Evidence supplied by Appellant" line so the report shows that
    // fact explicitly. If no appeal exists at all, skip the page entirely.
    try {
      ...
      if (compositeKey && jobData?.id) {
        ...
        if (appealRow) {
          ...
          const ratioStr = ratioDecimal ? `${(ratioDecimal * 100).toFixed(2)}% ${ratioSource}` : 'n/a';
          doc.setFontSize(9);
          ...
          if (appellantComps.length === 0) {
            ...
            // Lookup helpers (mirror AppellantEvidencePanel)
            const norm = (v) => String(v == null ? '' : v).trim().toUpperCase();
            const findCompProperty = (block, lot, qualifier, card) => {
              if (!block || !lot) return null;
              const b = norm(block), l = norm(lot), q = norm(qualifier), c = norm(card);
            ...
            // Sample range for evaluator
            const isLojikTenant = false;
            const sampleRange = (() => {
      ...
      // Don't block the export if the evidence page fails.
    }
