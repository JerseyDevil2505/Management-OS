        ...
        const HELP_TABS = [
          {
            ...
            intro:
              ...
              "Quick rule of thumb: most Towns flow Inspection Info → Market Analysis → Final Valuation. The CME (Comparable Market Evaluation) and the Appeal Log are the two pieces that can stand on their own — you can use them without going through the whole Market Analysis flow first.",
            steps: [
              { text: 'Top of the page is your home base. The Refresh button reloads the latest snapshot from the database. The Help button is what you are reading right now.', img: 'help/navigating/01-top-nav.png' },
              { text: 'Jobs is your dashboard. It lists every Town you have access to, shows status and freshness at a glance, and is also where you upload a new source or code file when your vendor sends one. Click any Town to open it.', img: 'help/navigating/02-jobs.png' },
              { text: 'When you open a Town you drop into the Town workspace. Across the top are the per-Town modules: Inspection Info, Data Visualizations, Market Analysis, and Final Valuation. Think of these as rooms inside the Town\'s house.', img: 'help/navigating/03-job-modules.png' },
              { text: 'Inspection Info is your read-only summary of inspection coverage — totals, by class, by VCS, by inspector, plus a printable PDF. Good for status meetings and sharing with the Town.', img: 'help/navigating/04-inspection-info.png' },
              { text: 'Data Visualizations gives you charts of the property mix and sales activity for the Town. It does not change anything — it is there to help you eyeball the data before you start analysis.', img: 'help/navigating/05-data-viz.png' },
              { text: 'Market Analysis is the multi-tab valuation prep workspace: Land Valuation (rates and brackets), Pre-Valuation (sales normalization), Attribute Cards, Overall Analysis, Data Quality, and Cost Valuation. Most of what you do here flows DOWNSTREAM into Final Valuation. Inside Data Quality there is a 📍 Coordinates sub-tab that lets you fix individual parcel lat/lng — see the Geocoding tab for how that works.', img: 'help/navigating/06-market-analysis.png' },
              { text: 'Final Valuation is where values actually get produced. The big pieces are: Sales Comparison / CME, the Appeal Log, the Detailed Appraisal Grid (manual appraisals with PDF export), Adjustments, Sales Review, Market Data, Vacant Land, Ratable Comparison, and Analytics. Most of these read from Market Analysis output, BUT — important — the CME tool and the Appeal Log can be run on their own. If you only need to handle a handful of appeals you do not need to do a full Market Analysis pass first.', img: 'help/navigating/07-final-valuation.png' },
              { text: 'Appeals (the top-level button) is a cross-Town summary that rolls up appeal stats from every Town you have access to.', img: 'help/navigating/08-appeals-summary.png' },
              { text: 'A note on data freshness: this app is NOT live-connected to BRT or Microsystems. Everything you see is the snapshot from your most recent source-file and code-file upload. When the Town\'s data changes on the vendor side, you re-upload the new file (see the "Updating your Town" tab) and the snapshot refreshes.', img: 'help/navigating/09-snapshot.png' },
            ],
          },
          {
            id: 'geocoding',
